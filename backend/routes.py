...
def get_model_info():
    if cached_model_info:
        return jsonify(cached_model_info), 200
...
def force_retrain():
    ...
    acquired = retrain_lock.acquire(blocking=False)
    if not acquired:
        return jsonify({"status": "success", "message": "Model retrain in progress (non-blocking)"}), 200
        
    try:
        if current_time - last_train_time > 10.0:
            train_model()
            # Reload model inside predictor
            predictor.load_model()
            load_cached_model_info()
            last_train_time = current_time
            msg = "Model retrained and reloaded successfully"
        else:
            msg = "Model retrain skipped (debounced) but server is healthy"
