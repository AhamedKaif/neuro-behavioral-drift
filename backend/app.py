...
def unauthorized_val(err):
    return jsonify({"error": err}), 401

@jwt.invalid_token_loader
def invalid_token_val(err):
    return jsonify({"error": err}), 401

@jwt.expired_token_loader
def expired_token_val(jwt_header, jwt_payload):
...
if __name__ == '__main__':
    ...
    print(f"Starting Flask backend server on port {port} using Waitress...")
    from waitress import serve
    serve(app, host='0.0.0.0', port=port, threads=128, connection_limit=1000, backlog=2048)
